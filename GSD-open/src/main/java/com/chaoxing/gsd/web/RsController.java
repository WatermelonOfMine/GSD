...
public class RsController{
    		...
    		@RequestParam(value = "datetimetype", required = true) String datetimetype) 
    {
    	...
    	if(!"0".equals(datetimetype) && !"1".equals(datetimetype) && !"2".equals(datetimetype) && !"3".equals(datetimetype))
        {
     ...
     * @param userId 用户id
     * @param type 查询类型（1是最近创建的5个标签，2是最近使用的5个标签）
     * @return
     */
    @GET
    ...
    //保存用户浏览记录(参数String userid,String content,String title1,String title2,String url)
     ...
     *@param ${userid}
     *@param ${content} 搜索内容
     *@param ${contentid} 搜索内容id
     *describe: 删除用户指定的搜索内容
     */
     ...
     *@param ${wiki}
     *@param ${message} 信息
     *@param ${updatetype} 更新方式，目前全部输入手动
     *@param ${userid}
     *@param ${csvurl} 库文件路径
     *describe: 用户提交库（索引内文档）
     */
    ...
    public BaseResponse PutIndex( GsdIndex gsdIndex){
        ...
        try{
			...
			msg.append("<b>联系电话:  </b>").append(StringUtil.isEmpty(gsdIndex.getTel()) ? "无" : gsdIndex.getTel()).append("<br><br>");
			msg.append("<b>邮箱:  </b>").append(gsdIndex.getEmail()).append("<br><br>");
			msg.append("<b>数据源Wiki:  </b>").append(StringUtil.isEmpty(gsdIndex.getWiki()) ? "无" : gsdIndex.getWiki()).append("<br><br>");
			msg.append("<b>文件地址:  </b>").append(gsdIndex.getCsvurl()).append("<br><br>");
			msg.append("<b>发送时间:  </b>").append(DateUtil.dateToString(new Date(), "yyyy-MM-dd HH:mm:ss")).append("<br><br>");
			msg.append("<b>其他信息:  </b>").append(StringUtil.isEmpty(gsdIndex.getMessage()) ? "无" : gsdIndex.getMessage()).append("<br>");
            new Thread(new Runnable()
            {
				...
				public void run() {
					...
					MailUtil.sendEmail(MailUtil.properties, msg.toString(), "text/html;charset=gb2312", "新数据源通知", "gsd@chaoxing.com", MailUtil.GSD_INQUIRY_MAIL, MailUtil.CC_MAIL_TO);
				}
    ...
    /**
     *@author heyang
     *@param ${userid}  传查找该用户提交的库，不传则查找所有用户提交的库
